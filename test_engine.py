...
# ---------------------------------------------------------------------------
# Robustness: parse_dates must NEVER leave the dataframe without a date
# column - this is what caused a real production crash
# ("AttributeError: 'Pandas' object has no attribute 'date'") deep inside
# the Sentiment Tracker tab when a CSV's date-like column had a name the
# code didn't recognize.
# ---------------------------------------------------------------------------
_no_date_df = pd.DataFrame({"home_team": ["A"], "away_team": ["B"]})
_no_date_result = E.parse_dates(_no_date_df, "date")
check("parse_dates guarantees a 'date' column exists even with no date-like column at all",
      "date" in _no_date_result.columns)
check("a genuinely dateless CSV gets NaT, not a crash",
      pd.isna(_no_date_result["date"].iloc[0]))

_altname_df = pd.DataFrame({"kickoff_time": ["2026-08-09"], "home_team": ["A"], "away_team": ["B"]})
_altname_result = E.parse_dates(_altname_df, "date")
check("parse_dates auto-detects a common alternate date column name (kickoff_time)",
      "date" in _altname_result.columns and pd.notna(_altname_result["date"].iloc[0]))
check("the auto-detected date value is actually correct, not just present",
      _altname_result["date"].iloc[0] == pd.Timestamp("2026-08-09"))

check("find_date_column returns None when nothing matches",
      E.find_date_column(pd.DataFrame({"home_team": ["A"]})) is None)
check("find_date_column finds 'match_date' as a valid alternate",
      E.find_date_column(pd.DataFrame({"match_date": ["x"]})) == "match_date")


# ---------------------------------------------------------------------------
# Live valuation-sheet market gate (Confidence Floor + Minimum EV%)
# ---------------------------------------------------------------------------
check("a market passes when it clears both confidence and EV thresholds",
      E.passes_market_gate(65, 5.0, 50, 3.0) is True)
check("a market is blocked when confidence alone is too low",
      E.passes_market_gate(40, 5.0, 50, 3.0) is False)
check("a market is blocked when EV alone is too low",
      E.passes_market_gate(65, 1.0, 50, 3.0) is False)
check("a market is blocked when BOTH confidence and EV are too low",
      E.passes_market_gate(40, 1.0, 50, 3.0) is False)
check("the gate uses >= at the exact boundary, not strict >",
      E.passes_market_gate(50, 3.0, 50, 3.0) is True)
check("a 0/0 gate still correctly blocks a genuinely negative-EV market",
      E.passes_market_gate(1, -50, 0, 0) is False)

check("market_gate_status reports 'Allowed' when both thresholds are cleared",
      E.market_gate_status(65, 5.0, 50, 3.0) == "✅ Allowed")
check("market_gate_status names confidence specifically when only that fails",
      E.market_gate_status(40, 5.0, 50, 3.0) == "⛔ Below Confidence Floor")
check("market_gate_status names EV specifically when only that fails",
      E.market_gate_status(65, 1.0, 50, 3.0) == "⛔ Below EV Gate")
check("market_gate_status names both when both fail",
      E.market_gate_status(40, 1.0, 50, 3.0) == "⛔ Below Confidence & EV")


# ---------------------------------------------------------------------------
# Odds calibration (tracks how often a team wins at a given odds/implied
# probability band, using only the model's own historical predictions)
# ---------------------------------------------------------------------------
check("implied_probability: 2.00 odds means exactly a 50% implied chance",
      abs(E.implied_probability(2.00) - 0.5) < 1e-9)
check("implied_probability: 4.00 odds means exactly a 25% implied chance",
      abs(E.implied_probability(4.00) - 0.25) < 1e-9)
check("implied_probability: zero/negative odds safely returns 0.0, not a crash",
      E.implied_probability(0) == 0.0)

_calib_df = pd.DataFrame([
    {"p_home": 0.50, "p_draw": 0.25, "p_away": 0.25, "actual": "H", "correct": True},
    {"p_home": 0.20, "p_draw": 0.30, "p_away": 0.50, "actual": "A", "correct": True},
    {"p_home": 0.55, "p_draw": 0.25, "p_away": 0.20, "actual": "D", "correct": False},
    {"p_home": 0.10, "p_draw": 0.15, "p_away": 0.75, "actual": "H", "correct": False},
])
_calib_table = E.odds_calibration_table(_calib_df)
check("odds_calibration_table groups the two 50%-confidence picks and the 55% pick into the 1.51-2.00 band",
      int(_calib_table.loc[_calib_table["Odds Band"] == "1.51-2.00", "Predictions"].iloc[0]) == 3)
check("odds_calibration_table's actual win rate matches a hand-counted result (2 of 3 correct = 66.7%)",
      abs(_calib_table.loc[_calib_table["Odds Band"] == "1.51-2.00", "Actual Win Rate %"].iloc[0] - 66.7) < 0.1)
check("odds_calibration_table returns an empty (not crashing) frame for an empty backtest",
      E.odds_calibration_table(pd.DataFrame()).empty)


# ---------------------------------------------------------------------------
# Cross-league bet slip: combine_parlay_legs must work identically whether
# fed MarketRow objects (single-fixture parlay) or plain dicts (the
# persisted multi-league slip, which outlives any single fixture's
# MarketRow objects)
# ---------------------------------------------------------------------------
_row_a = E.MarketRow(market="Home Win", bookmaker_odds=2.5, dc_prob=0.5, mc_prob=0.48,
                      convergence=0.98, fair_odds=2.0, ev=0.25, volatility_tier="Balanced",
                      verdict="ELITE", recommended_action="BET")
_row_b = E.MarketRow(market="BTTS Yes", bookmaker_odds=1.8, dc_prob=0.55, mc_prob=0.57,
                      convergence=0.98, fair_odds=1.75, ev=0.026, volatility_tier="Balanced",
                      verdict="TRAP", recommended_action="SKIP")
_dict_a = {"league": "EPL", "fixture": "A vs B", "market": "Home Win", "bookmaker_odds": 2.5, "dc_prob": 0.5, "mc_prob": 0.48, "ev": 0.25}
_dict_b = {"league": "Bundesliga", "fixture": "C vs D", "market": "BTTS Yes", "bookmaker_odds": 1.8, "dc_prob": 0.55, "mc_prob": 0.57, "ev": 0.026}

_odds_obj, _prob_obj = E.combine_parlay_legs([_row_a, _row_b])
_odds_dict, _prob_dict = E.combine_parlay_legs([_dict_a, _dict_b])
_odds_mixed, _prob_mixed = E.combine_parlay_legs([_row_a, _dict_b])
check("combine_parlay_legs gives identical results for MarketRow objects and equivalent plain dicts",
      abs(_odds_obj - _odds_dict) < 1e-9 and abs(_prob_obj - _prob_dict) < 1e-9)
check("combine_parlay_legs also works with a MIXED list of MarketRow objects and dicts",
      abs(_odds_mixed - _odds_obj) < 1e-9 and abs(_prob_mixed - _prob_obj) < 1e-9)
check("combine_parlay_legs correctly multiplies odds across legs (2.5 x 1.8 = 4.5)",
      abs(_odds_obj - 4.5) < 1e-9)


# ---------------------------------------------------------------------------
# League reliability badge
# ---------------------------------------------------------------------------
check("league_reliability_badge: clearly positive BSS is Reliable (green)",
      E.league_reliability_badge(0.05) == ("🟢", "Reliable"))
check("league_reliability_badge: BSS just above zero is Marginal (yellow), not Reliable",
      E.league_reliability_badge(0.01) == ("🟡", "Marginal"))
check("league_reliability_badge: BSS just below zero is still Marginal, not Unreliable",
      E.league_reliability_badge(-0.01) == ("🟡", "Marginal"))
check("league_reliability_badge: clearly negative BSS is Unreliable (red)",
      E.league_reliability_badge(-0.05) == ("🔴", "Unreliable"))
check("league_reliability_badge: NaN (not enough data) gets its own neutral badge, not a red warning",
      E.league_reliability_badge(float("nan")) == ("⚪", "Insufficient Backtest Data"))
check("league_reliability_badge: None is treated the same as NaN",
      E.league_reliability_badge(None) == ("⚪", "Insufficient Backtest Data"))


# ---------------------------------------------------------------------------
# Per-league territory weight calibration - verified against a SYNTHETIC
# league where box touches is deliberately the only real signal (big
# chances and shots on target are pure noise), so a correct calibration
# should identify and lean toward it.
# ---------------------------------------------------------------------------
_rng = np.random.default_rng(3)
_teams = ["Strong", "Weak", "Mid1", "Mid2", "Mid3", "Mid4"]
_rows = []
_start = pd.Timestamp("2025-08-01")
_day = 0
for _ in range(6):
    for _home in _teams:
        for _away in _teams:
            if _home == _away:
                continue
            _day += 1
            _date = _start + pd.Timedelta(days=_day)
            _home_box = _rng.normal(28 if _home == "Strong" else (10 if _home == "Weak" else 18), 2)
            _away_box = _rng.normal(28 if _away == "Strong" else (10 if _away == "Weak" else 18), 2)
            _home_bc, _away_bc = _rng.poisson(1.5), _rng.poisson(1.5)
            _home_sot, _away_sot = _rng.poisson(4), _rng.poisson(4)
            _home_goals = _rng.poisson(max(0.1, _home_box * 0.05))
            _away_goals = _rng.poisson(max(0.1, _away_box * 0.05))
            _rows.append({
                "date": _date.strftime("%Y-%m-%d"), "league_country": "Signal Test League",
                "home_team": _home, "away_team": _away, "home_goals": _home_goals, "away_goals": _away_goals,
                "home_shots_on_target": _home_sot, "away_shots_on_target": _away_sot,
                "home_big_chances": _home_bc, "away_big_chances": _away_bc,
                "home_box_touches": _home_box, "away_box_touches": _away_box,
            })
_raw_signal_df = pd.DataFrame(_rows)
_signal_df = E.standardise_columns(_raw_signal_df)
for _c in ["home_goals", "away_goals", "home_shots_on_target", "away_shots_on_target",
           "home_big_chances", "away_big_chances", "home_box_touches", "away_box_touches"]:
    _signal_df[_c] = pd.to_numeric(_signal_df[_c], errors="coerce")
_signal_df = E.parse_dates(_signal_df, "date")
_signal_settled, _ = E.split_played_unplayed(_signal_df)
_signal_half_life, _ = E.optimize_half_life(_signal_settled)
_calibrated_weights, _calibration_info = E.optimize_territory_weights(_signal_settled, _signal_half_life)

check("optimize_territory_weights identifies box touches as dominant when it's the only real signal",
      _calibrated_weights[2] >= 0.35)
check("optimize_territory_weights never scores worse than equal weighting (equal weights is always a candidate)",
      _calibration_info["equal_weight_score"] >= _calibration_info["brier_scores_by_weights"][_calibration_info["chosen"]])
check("optimize_territory_weights falls back to equal weights with a clear reason on too little data",
      E.optimize_territory_weights(_signal_settled.head(3), _signal_half_life)[0] == E.TERRITORY_WEIGHTS_DEFAULT)

check("attack_strength accepts a custom weights tuple and actually uses it (not silently ignored)",
      E.attack_strength(
          E.team_territory_profile(_signal_settled, "Strong", "home", _signal_half_life, _signal_settled["date"].max()),
          E.compute_league_baseline(_signal_settled), "home", (0.0, 0.0, 1.0),
      ) != E.attack_strength(
          E.team_territory_profile(_signal_settled, "Strong", "home", _signal_half_life, _signal_settled["date"].max()),
          E.compute_league_baseline(_signal_settled), "home", (1.0, 0.0, 0.0),
      ))


# ---------------------------------------------------------------------------
# Half-life search-boundary warning
# ---------------------------------------------------------------------------
check("half_life_at_search_boundary warns when the optimizer picks the exact MAXIMUM tested (180)",
      E.half_life_at_search_boundary(180) is not None and "LONGEST" in E.half_life_at_search_boundary(180))
check("half_life_at_search_boundary warns when the optimizer picks the exact MINIMUM tested (15)",
      E.half_life_at_search_boundary(15) is not None and "SHORTEST" in E.half_life_at_search_boundary(15))
check("half_life_at_search_boundary stays silent for a half-life comfortably inside the range",
      E.half_life_at_search_boundary(60) is None)
check("half_life_at_search_boundary is safe against NaN/None rather than crashing",
      E.half_life_at_search_boundary(float("nan")) is None and E.half_life_at_search_boundary(None) is None)


# ---------------------------------------------------------------------------
# Safety-rail dilution report - checks how much of the half-life search
# window is diluted by thin (< 5 match) per-team-venue samples
# ---------------------------------------------------------------------------
_rng2 = np.random.default_rng(5)
_sparse_teams = ["A", "B", "C", "D", "E", "F", "G", "H"]
_sparse_rows = []
_start2 = pd.Timestamp("2025-08-01")
_day2 = 0
for _home in _sparse_teams:
    for _away in _sparse_teams:
        if _home == _away:
            continue
        _day2 += 1
        _sparse_rows.append({
            "date": _start2 + pd.Timedelta(days=_day2), "home_team": _home, "away_team": _away,
            "home_goals": _rng2.poisson(1.3), "away_goals": _rng2.poisson(1.1),
            "home_shots_on_target": _rng2.poisson(5), "away_shots_on_target": _rng2.poisson(4),
            "home_big_chances": _rng2.poisson(2), "away_big_chances": _rng2.poisson(1.5),
            "home_box_touches": _rng2.poisson(20), "away_box_touches": _rng2.poisson(18),
        })
_sparse_df = pd.DataFrame(_sparse_rows)
_sparse_report = E.safety_rail_dilution_report(_sparse_df, max_matches_evaluated=150)
check("safety_rail_dilution_report shows heavy dilution for a genuinely sparse single round-robin league",
      _sparse_report["diluted_pct"] > 50)

_rich_teams = [f"T{i}" for i in range(18)]
_rich_rows = []
_day3 = 0
for _ in range(2):
    for _home in _rich_teams:
        for _away in _rich_teams:
            if _home == _away:
                continue
            _day3 += 1
            _rich_rows.append({
                "date": _start2 + pd.Timedelta(days=_day3), "home_team": _home, "away_team": _away,
                "home_goals": _rng2.poisson(1.3), "away_goals": _rng2.poisson(1.1),
                "home_shots_on_target": _rng2.poisson(5), "away_shots_on_target": _rng2.poisson(4),
                "home_big_chances": _rng2.poisson(2), "away_big_chances": _rng2.poisson(1.5),
                "home_box_touches": _rng2.poisson(20), "away_box_touches": _rng2.poisson(18),
            })
_rich_df = pd.DataFrame(_rich_rows)
_rich_report = E.safety_rail_dilution_report(_rich_df, max_matches_evaluated=150)
check("safety_rail_dilution_report shows low/no dilution for a data-rich multi-season league",
      _rich_report["diluted_pct"] < 20)
check("safety_rail_dilution_report handles a genuinely tiny dataset without crashing",
      "reason" in E.safety_rail_dilution_report(pd.DataFrame({"date": [], "home_team": [], "away_team": []})))


# ---------------------------------------------------------------------------
# Extended half-life diagnostic (checks past the normal 180-day ceiling
# without changing what the live app actually searches by default)
# ---------------------------------------------------------------------------
check("EXTENDED_HALF_LIFE_CANDIDATES genuinely extends past the normal 180-day ceiling",
      max(E.EXTENDED_HALF_LIFE_CANDIDATES) > max(E.HALF_LIFE_CANDIDATES))
check("EXTENDED_HALF_LIFE_CANDIDATES still includes every normal candidate (a strict extension, not a replacement)",
      set(E.HALF_LIFE_CANDIDATES).issubset(set(E.EXTENDED_HALF_LIFE_CANDIDATES)))

# A genuine quality-reversal league (Strong and Weak swap identity partway
# through the season) - old pre-swap data becomes actively MISLEADING, so
# a real recency effect exists and long memory should NOT keep winning.
_rng4 = np.random.default_rng(11)
_teams4 = ["Strong", "Weak", "Mid1", "Mid2", "Mid3", "Mid4"]
_rows4 = []
_start4 = pd.Timestamp("2025-08-01")
_day4 = 0
_match_idx4 = 0
for _ in range(8):
    for _home in _teams4:
        for _away in _teams4:
            if _home == _away:
                continue
            _day4 += 1
            _match_idx4 += 1
            _date4 = _start4 + pd.Timedelta(days=_day4)
            _eh, _ea = _home, _away
            if _match_idx4 > 100:  # quality swap partway through
                _swap4 = {"Strong": "Weak", "Weak": "Strong"}
                _eh, _ea = _swap4.get(_home, _home), _swap4.get(_away, _away)
            _hbox4 = _rng4.normal(28 if _eh == "Strong" else (10 if _eh == "Weak" else 18), 2)
            _abox4 = _rng4.normal(28 if _ea == "Strong" else (10 if _ea == "Weak" else 18), 2)
            _hbc4, _abc4 = _rng4.poisson(1.5), _rng4.poisson(1.5)
            _hsot4, _asot4 = _rng4.poisson(4), _rng4.poisson(4)
            _hgoals4 = _rng4.poisson(max(0.1, _hbox4 * 0.05))
            _agoals4 = _rng4.poisson(max(0.1, _abox4 * 0.05))
            _rows4.append({
                "date": _date4.strftime("%Y-%m-%d"), "league_country": "Reversal Test League",
                "home_team": _home, "away_team": _away, "home_goals": _hgoals4, "away_goals": _agoals4,
                "home_shots_on_target": _hsot4, "away_shots_on_target": _asot4,
                "home_big_chances": _hbc4, "away_big_chances": _abc4,
                "home_box_touches": _hbox4, "away_box_touches": _abox4,
            })
_reversal_df = E.standardise_columns(pd.DataFrame(_rows4))
for _c in ["home_goals", "away_goals", "home_shots_on_target", "away_shots_on_target",
           "home_big_chances", "away_big_chances", "home_box_touches", "away_box_touches"]:
    _reversal_df[_c] = pd.to_numeric(_reversal_df[_c], errors="coerce")
_reversal_df = E.parse_dates(_reversal_df, "date")
_reversal_settled, _ = E.split_played_unplayed(_reversal_df)
_reversal_result = E.extended_half_life_diagnostic(_reversal_settled)

check("extended_half_life_diagnostic returns a real verdict (not None) for a large-enough dataset",
      _reversal_result["still_improving_past_180"] is not None)
check("extended_half_life_diagnostic correctly finds NO benefit past 180 when there's a genuine quality reversal "
      "(old data is actively misleading there, so long memory shouldn't keep winning)",
      _reversal_result["still_improving_past_180"] is False)
check("extended_half_life_diagnostic falls back cleanly with a reason on too little data, rather than crashing",
      E.extended_half_life_diagnostic(_reversal_settled.head(3))["still_improving_past_180"] is None)


# ---------------------------------------------------------------------------
# Per-market backtest accuracy (all 22 markets, not just the outright 1X2
# pick) - this had zero direct test coverage despite being fully wired
# into the app, which is exactly the pattern that's caused real bugs
# before in this project.
# ---------------------------------------------------------------------------
check("per_market_backtest_accuracy returns an empty dict for an empty backtest, not a crash",
      E.per_market_backtest_accuracy(pd.DataFrame()) == {})
check("per_market_backtest_accuracy returns an empty dict when include_all_markets wasn't used (no correct__ columns)",
      E.per_market_backtest_accuracy(pd.DataFrame({"p_home": [0.5], "actual": ["H"]})) == {})

check("actual_market_outcomes covers every single market in MARKET_LIST (no silent gaps)",
      set(E.MARKET_LIST) - set(E.actual_market_outcomes(2, 1).keys()) == set())
check("market_probs_from_matrix covers every single market in MARKET_LIST (no silent gaps)",
      set(E.MARKET_LIST) - set(E.market_probs_from_matrix(E.build_score_matrix(1.4, 1.1, 0.0)).keys()) == set())

_pm_rng = np.random.default_rng(11)
_pm_teams = [f"Team{i}" for i in range(8)]
_pm_rows = []
_pm_start = pd.Timestamp("2025-08-01")
_pm_day = 0
for _ in range(4):
    for _home in _pm_teams:
        for _away in _pm_teams:
            if _home == _away:
                continue
            _pm_day += 1
            _pm_rows.append({
                "date": (_pm_start + pd.Timedelta(days=_pm_day)).strftime("%Y-%m-%d"),
                "league_country": "Test League", "home_team": _home, "away_team": _away,
                "home_goals": _pm_rng.poisson(1.4), "away_goals": _pm_rng.poisson(1.1),
                "home_shots_on_target": _pm_rng.poisson(5) + 1, "away_shots_on_target": _pm_rng.poisson(4) + 1,
                "home_big_chances": _pm_rng.poisson(2), "away_big_chances": _pm_rng.poisson(1.5),
                "home_box_touches": _pm_rng.poisson(20) + 10, "away_box_touches": _pm_rng.poisson(16) + 10,
            })
_pm_raw = pd.DataFrame(_pm_rows)
_pm_df = E.standardise_columns(_pm_raw)
for _c in ["home_goals", "away_goals", "home_shots_on_target", "away_shots_on_target",
           "home_big_chances", "away_big_chances", "home_box_touches", "away_box_touches"]:
    _pm_df[_c] = pd.to_numeric(_pm_df[_c], errors="coerce")
_pm_df = E.parse_dates(_pm_df, "date")
_pm_settled, _ = E.split_played_unplayed(_pm_df)
_pm_full_bt = E.walk_forward_backtest(_pm_settled, half_life_days=45, include_all_markets=True)
_pm_accuracy = E.per_market_backtest_accuracy(_pm_full_bt)

check("per_market_backtest_accuracy returns all 22 markets when given a real include_all_markets=True backtest",
      len(_pm_accuracy) == 22)
check("every per-market accuracy value is a valid percentage between 0 and 100",
      all(0 <= v <= 100 for v in _pm_accuracy.values()))
check("walk_forward_backtest with include_all_markets=False (the default) does NOT produce per-market columns",
      E.per_market_backtest_accuracy(E.walk_forward_backtest(_pm_settled, half_life_days=45)) == {})
